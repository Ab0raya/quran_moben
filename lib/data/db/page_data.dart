...
List<Map<String, dynamic>> surahStartPages = [];

void extractSurahStartPages() {
  for (int pageIndex = 0; pageIndex < pageData.length; pageIndex++) {
    for (var detail in pageData[pageIndex]) {
      if (surahStartPages
              .indexWhere((surah) => surah['surah'] == detail['surah']) ==
          -1) {
        surahStartPages.add({
          'surah': detail['surah'],
          'page': pageIndex + 1 // Pages are 1-based index
        });
      }
    }
  }
